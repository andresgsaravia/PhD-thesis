...
\begin{table}[ht]
  ...
  \begin{tabular}[ht]{|l|c|c|c|c|}
    ...
    \hline
    \end{tabular}
    \caption[Isotopic changes in the infrared excitations for the $\lambda_{ir}=0.1263$ eV case.]
    {Isotopic changes in the infrared excitations for the $\lambda_{ir}=0.1263$ eV case compared with the harmonic prediction.}
...
It was shown in Figure \ref{fig:irSpectra} that for large $\lambda_{ir}$ the energy of both states with two and three infrared phonons converge to $\omega_{ir}$.
...
This is further evidence of the charge \textit{freezing} and the appereance of static distortions in the cluster.

Since the states with an even number of infrared phonons converge to states with an odd number of infrared phonons, at some point in the $\lambda_{ir}$ regime they must develop an additional node.
It was shown in \ref{fig:uirCoupl} that the ground state, for $\lambda_{ir}=0.1263$ eV, has already developed a peak separation that becomes complete for $\lambda_{ir}>0.16$ eV.
In contrast, the excitation with two infrared phonons has still only one central peak for $\lambda_{ir}=0.1263$ eV and develops the additional node for greater coupling values.