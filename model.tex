...
\end{equation}
%
with $m_{ir}$ and $m_R$ being the reduced mass for each mode (see (\ref{eq:redMassR}, \ref{eq:redMassIr})), $x_i=2$ the position of the central copper atom and $x_i=1,3$ the positions of the oxygen atoms.
The constant $s_0$ is introduced to avoid artificial shrinking of the cluster\footnote{This term is written slightly different in some publications, like \cite{MustredeLeon1992}, however both versions can be related noticing that $3 (n_1+n_3-4/3)=n_1-2n_2+n_3$.}. 
...
To model the effect of this change in mass we describe the motion of the nuclei as three masses ($m_i,\ i=1,2,3$) attached by springs of constants $k_1$ and $k_2$ (see figure \ref{fig:3-masses-2-springs}) and find the normal vibrational modes using classical mechanics.
In our particular case we are interested in $m_1=m_3$ and $k_1=k_2$.
%
\begin{figure}[ht!]
  \centering
...
%
for some constant $k$.
From here we identify the reduced masses for each vibrational mode
%
\begin{equation}
  \label{eq:redMassR}
...
Denoting as $\omega^{(16)}$ and $\omega^{(18)}$ the phonon frequencies for a cluster with $^{16}$O and $^{18}$O respectively, from (\ref{eq:omegair}, \ref{eq:omegaR}) we can find the ratios
%
\begin{align}
...
\end{equation}
%
where the energies $\omega_i$ are referred to the corresponding ground state.
From this definition, using (\ref{eq:omega-ir-isot}, \ref{eq:omega-R-isot}), we can calculate the isotopic shift predicted by harmonic potentials for the Raman and infrared modes
%
\begin{equation}
  \label{eq:isot-harm-R}
  \Delta_{R} = 1 - \frac{\omega_{R}(^{18})}{\omega_{R}(^{16})} \times 100 = 5.75\% 
\end{equation}
%
\begin{equation}
  \label{eq:isot-harm-ir}
  \Delta_{ir} = 1 - \frac{\omega_{ir}(^{18})}{\omega_{ir}(^{16})} \times 100 = 3.75\%
\end{equation}
%
...
\end{equation}
%
and we can make the partial projection into the phonon coordinates $(u_{ir},u_R)$ as defined by (\ref{eq:uR},\ref{eq:uir})
%
\begin{equation}
...
\end{equation}
%
where we have denoted $\braket{u_{ir},u_r}{e_1,e_2,ir,R}=\braket{u_{ir},u_R}{ir,R}\ket{e_1,e_2}$. 
